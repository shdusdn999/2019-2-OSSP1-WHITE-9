...
class NoisyImageGenerator(Sequence):
    def __init__(self, image_dir, source_noise_model, target_noise_model, batch_size=32, image_size=64):
        image_suffixes = (".jpeg", ".jpg", ".png", ".bmp")  #왼쪽의 4개의 확장자로 이미지 파일을 생성함.
        ...
        if self.image_num == 0: #해당 경로에 이미지가 존재하지 않을 경우 에러메시지 출력
...
class ValGenerator(Sequence):   #해당 이미지를 수치화함.
